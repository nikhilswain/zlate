...
export function ProjectDeleteModal() {
  ...
  return (
    <AnimatePresence>
      {open && project && <ModalInner key={project.id} project={project} />}
    </AnimatePresence>
  );
}

function ModalInner({ project }: { project: Project }) {
      ...
      >
        ...
        <header className="flex items-center justify-between px-5 py-3.5 border-b border-border-subtle">
          <div className="flex items-center gap-2.5">
            ...
            />
            <div className="text-sm font-medium text-fg">Delete project</div>
          </div>
          <button
            type="button"
            onClick={cancel}
